...
public class PostController {
	...
	@PostMapping("/Post")
	  public ResponseEntity<PostEntity> createTutorial(@RequestBody PostEntity p) {
	    try {
	      PostEntity prod = postService.create(new PostEntity(p.getId(), p.getNombre(), p.getDescripcion(),p.getPrecio(), p.getStock(), p.getEstado()));
	...
	@PutMapping("/prod/{id}")
	  public ResponseEntity<PostEntity> updateTutorial(@PathVariable("id") long id, @RequestBody PostEntity p) {
	    ...
	    if (post!=null) {
	      ...
	      post.setPrecio(p.getPrecio());
	      post.setStock(p.getStock());
